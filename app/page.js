...
export default function Home() {
  ...
  const month = selected && selected?.toLocaleString('default',{month: 'long'});
    const dayOfWeek = selected && selected?.toLocaleString('default',{weekday: 'long'});
  const day= selected?.getDate();
  const year = selected?.getFullYear();
  // console.log(typeof selected, selected.toLocaleString())
  return (
    <div className="flex gap-4 py-10 flex-col md:flex-row sm:flex-col h-[100%]">
      <Calendar 
      mode="single"
      selected = {selected}
      ...
      ></Calendar>
        <Card className={"md:h-[calc(100%-100px)] sm:h-[calc(100%-350px)] mx-5 md:flex-1"}>
          <CardHeader>
         ...
         {/* <ul className="flex justify-between p-10">
          ...
          <li><Button variant={"outline"}>Exit</Button></li>
            ...
            <ul className="flex gap-2 justify-between px-4">
              <li className="border my-auto rounded-2xl p-3 flex gap-3"> 
                <Badge variant="default">Time in</Badge> <p>{currentDate.toLocaleTimeString()}</p>
              </li>
            ...
            <StaticTimePicker defaultValue={dayjs('2022-04-17T15:30')} />
              ...
              <li className="border my-auto rounded-2xl p-3 flex gap-3"> 
                <Badge variant="default">Time out</Badge> <p>{currentDate.toLocaleTimeString()}</p>
              </li>
            </ul>
